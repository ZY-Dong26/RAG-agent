# 06_generator.py —— 生成层：检索到的chunk上下文 + 用户问题 → 云端LLM → 答案
# 关键设计：
#   1. LLM走OpenAI兼容协议：云端大模型（DeepSeek/通义/Kimi/OpenAI等）基本都支持，
#      换厂商只改 00_config.py 里的 LLM_BASE_URL / LLM_API_KEY / LLM_MODEL 三件套
#   2. client可注入（依赖注入）：测试时传假client，不真的调云端；平时自动按config创建
#   3. prompt = 系统约束(只依据资料回答) + 编号资料 + 问题，句末用[编号]标注引用
#   4. 资料拼接有字符上限(MAX_CONTEXT_CHARS)，防止超出模型窗口
...
class Generator:
    ...
    def build_prompt(query, hits, max_chars=None):
        ...
        :param query: 用户问题
        :param hits: 05_retriever的返回（含text/metadata/score/rank）
        :param max_chars: 资料总字符上限，超限从后面截断
        :return: messages，[{"role":"system",...}, {"role":"user",...}]
        """
        max_chars = max_chars or config.MAX_CONTEXT_CHARS

        # 把命中的chunk按排名拼成"编号+正文"的资料块，总量不超过max_chars
        blocks = []
        total = 0
        for hit in hits:
            rank = hit["metadata"].get("rank", "?")
            block = f"[{rank}] {hit['text'].strip()}"
            if total + len(block) > max_chars:   # 超限截断，防超模型窗口
                break
            blocks.append(block)
            total += len(block)
        context = "\n\n".join(blocks)
    ...
    def generate(self, query, hits):
        """
        生成回答：hits(05检索结果) + query → 云端LLM → 答案文本
        :param query: 用户问题
        :param hits: 05_retriever.retrieve()的返回列表
        :return: 回答字符串（可能含[编号]引用标注）
        """
        messages = self.build_prompt(query, hits)

        # OpenAI兼容接口的标准调用：各家云端都认这个写法
